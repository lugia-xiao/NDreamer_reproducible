...
def positive_true_positive(adata, batch_key='batch', celltype_key='celltype', use_raw=False, k1=20, k2=100, tp_thr=3.,
                           distance='cosine', embed='X_pca'):
...
def calculate_metrics(adata, batch_key='batch', celltype_key='celltype', all=False,
                      savepath=None, subsample=None, seed=0, tp_thr=3., is_raw=False, n_neighbors=15, embed='X_pca'
                      ):
    if subsample:
        random.seed(seed)
        np.random.seed(seed)
        ...
        if not is_raw:
            raise ValueError("No is_raw=False")
        del sample_idx
    ...
    if total_cells<100:
        print("!"*40)
        print("Too few number of cells for one condition or batch")
        print("return")
        print("!"*40)
        return
    ...
    if all:
        ...
        print("cLISI, bLISI, LISI_F1",cLISI, bLISI, LISI_F1)
    ...
    #p_kbet=scib_metrics.kbet(X=adata.uns['neighbors'],batches=adata.obs[batch_key])
    #print("p_kbet",p_kbet)
