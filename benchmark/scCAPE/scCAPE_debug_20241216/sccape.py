...
def reconstruction_errors(X, loadings, njobs=2):
    ...
    for j in range(len(loadings)):  # For each factor loading matrix j in loadings list
        ...
        errors.append(
            mean_squared_error(np.asarray(D),np.asarray(Dj)))  # compute mean squared error between original data D and reconstructed data Dj
    return errors, projs
...
def find_best_k(oNMFres, dataset_path, alpha=1, SS_weight=0.3):
    ...
    for i in range(len(alphas)):
        ...
        for j in jrange:
            ...
            fk_df = pd.concat([fk_df, curr_df], ignore_index=True)
