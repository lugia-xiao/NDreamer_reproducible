...
generate_radar_plot <- function(df,title) {
  ...
  # Group definitions
  groups <- list(
    "EM-Bio conservation" = c("ASW_label", "ARI", "NMI", "cLISI"),
  ...
  group_means <- df %>%
    rowwise() %>%
    mutate(
      `EM-Bio conservation` = mean(c_across(all_of(groups$`EM-Bio conservation`)), na.rm = TRUE),
    ...
    ungroup() %>%
    select(Method, `EM-Bio conservation`, `EM-batch condition mix`, `Denoised-batch mixing`, 
  ...
  radar_data<-radar_data[,c(5,4,3,1,2)]
  # Rename the column
...
# Example usage
df <- read.csv("./case_control/ECCITE.csv")
generate_radar_plot(df,"ECCITE")

df <- read.csv("./case_control/ASD.csv")
generate_radar_plot(df,"ASD")

df <- read.csv("./case_control/t1d.csv")
generate_radar_plot(df,"T1D")

df <- read.csv("./case_control/kidney.csv")
generate_radar_plot(df,"Kidney")
