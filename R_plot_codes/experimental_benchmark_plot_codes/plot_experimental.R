...
generate_plot <- function(input_df) {
  ...
  plot_data <- input_df %>%
    ...
    filter(!is.na(Value), Metric %in% c(batch_correction_metrics, biological_conservation_metrics)) %>%
    group_by(Metric) %>%
  ...
  plot_data <- plot_data %>%
    mutate(Category = case_when(
      Metric %in% batch_correction_metrics ~ "Condition Mixing",
  ...
  p <- ggplot(plot_data, aes(x = Metric, y = Method, fill = RelativeValue)) +
    geom_point(shape = 21, size = 21, color = "black") +  # Increase circle size
