    ...
    def predict_counts(self, input_counts, meta_cells, condition_keys, batch_key, predict_conds = None, predict_batch = None):
        ...
        for idx, condition_key in enumerate(condition_keys):
            ...
            for condition_id, condition in enumerate(self.data_dict["matching_dict"]["cond_names"][idx]):
                ...
                curr_cond[meta_cells[condition_key].to_numpy().flatten() == condition] = condition_id
        ...
        for batch_id, batch in enumerate(self.data_dict["matching_dict"]["batch_name"]):
            ...
            curr_batch[meta_cells[batch_key].to_numpy().flatten() == batch] = batch_id
        ...
        with torch.no_grad():
            ...
            for dataset in self.data_dict["datasets"]:
                ...
                dict_inf_train = self.inference(counts = dataset.counts_norm.to(self.device), batch_ids = dataset.batch_id[:,None].to(self.device), print_stat = False)
            ...
            dic_inf_pred = self.inference(counts = counts_norm.to(self.device), batch_ids = curr_batch[:, None].to(self.device), print_stat = False)
